...
function wxParse(bindName = 'wxParseData', type='html', data='<div class="color:red;">数据不能为空</div>', target,imagePadding) {
  var that = target;
  data = wxDiscode.strcharacterDiscode(data);
  ...
  if (type == 'html') {
    ...
    if (p.attr.src.indexOf('https') === -1) {
      p.attr.src = "http://192.168.0.101" + p.attr.src;
    }
    }
    }
    app.globalData.imageUrls = []
    for (var i = 0; i < transData.imageUrls.length; i++) {

          
    var obj = {};
      transData.imageUrls[i] = "http://192.168.0.101" + transData.imageUrls[i];
    ...
    console.log(app.globalData.imageUrls)
    }
    // app.globalData.imageUrls=[]
